...
import { clearParameters, getListsOfRegion } from "store/parameter";
import { getListsAdditional } from "store/code";
import { generateReport, getListsCOCFile, getReportParametter, clearReportParametter } from "store/coc";
import "./style.css";
import arrow_right_icon from "assets/images/arrow-right-icon.png";
import { clearParametersWithoutCoC } from "store/file";

type Object = {
    [key: string]: any;
};

export function ReportParametersModal({ cocID, open, closeModal }) {
    ...
    const [codes, parameters, loadingParameter, additionalCodes, loading, reportParamaters, reportLoading] = useAppSelector((state) => [state?.code?.codes, state?.parameter?.parameters, state?.parameter?.loading, state?.code?.additionalCodes, state?.coc?.loading, state?.coc?.parameters, state?.coc?.reportLoading]);
    ...
    useEffect(() => {
        dispatch(getReportParametter(cocID));
        return () => {
            dispatch(clearParameters())
            dispatch(clearReportParametter())
    ...
    const onSubmit = async (data) => {
        ...
        const report = await dispatch(generateReport({ id: cocID || 0, data: submitData }));
        if (report?.meta?.requestStatus === "fulfilled") {
            toast.success('Generate Report successfully!');
            if (report?.payload?.report_lab_url !== '') {
                downloadFile(report?.payload?.report_lab_url || "");
            }
            reset();
            closeModal();
