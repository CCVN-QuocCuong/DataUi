...
import { getCOCFile, getListsCOCFile, getFileDetail, uploadCOCFiles, removeAllCOCFile, removeCOCFile, setUploadSuccess, uploadCOCFileToS3, getListSamplesFile, acceptMappingSample, rejectMappingSample, getMappingSample } from "store/coc";
...
export function COCFiles({ cocID }) {
    ...
    const [listLoading, loading, files] = useAppSelector((state) => [state.coc.listFilesloading, state.coc.loading, state.coc.files]);
    ...
    useEffect(() => {
        const listFiles = files?.filter((file: Object) => file?.filetype !== 'application/pdf' && file?.filetype !== 'json') || [];
        setRows(listFiles);
        return () => {
            setSelectedFile('')
        }
    ...
    /**
     * Handle close modal confirm mapping sample
     */
    const handleCloseModalConfirmMappingSample = useCallback(async () => {
        await dispatch(rejectMappingSample(currentFile?.header?.[0]?.fileid || 0));
        await dispatch(getListsCOCFile(cocID));
        setShowModalConfirmMappingSample(false);
    }, [currentFile]);

    /**
     * Handle close modal mapping sample
     * @type {*}
     */
    const handleCloseModalMappingSample = useCallback(() => {
    ...
    const handleUploadFile = async (e, type) => {
        ...
        if (file) {
            if (file.type && (type === 'csv' && file.type !== "text/csv")) {
