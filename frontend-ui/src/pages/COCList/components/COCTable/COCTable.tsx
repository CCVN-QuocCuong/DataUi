...
export function COCTable({ type }) {
    const dispatch = useAppDispatch();
    const history = useHistory();

    const [rows, setRows] = useState<object[]>([]);
    const [paramQuery, setParamQuery] = useState<Object>({});
    const { loading, pagination } = useAppSelector((state) => state.coc);
    ...
    useEffect(() => {
        const typeParam = type === 'soil' ? "Soil" : (type === 'liquid' ? 'Liquid' : 'Landfill Gas')
        const param = {
            ...paramQuery,
            sampletype: typeParam,
            page: 1,
            pagesize: pagination?.pageSize || 10,
        }
        dispatch(getLists(param)).then((res) => {
    ...
    /**
     * Get data list by page number when click button next or prev page
     * @param {Number} page
     */
    ...
    /**
     * Get data list by rows per page 
    ...
    return (
        <div className="coc-table-wrapper">
            <SearchBar sampleType={type} handleSearchSample={handleSearchSample} />
